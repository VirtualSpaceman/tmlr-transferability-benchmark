...
MODEL_TO_MARKERS = rodict({
    'efficientnet_b0':       SINGLE_MARKER,
    'densenet121':           SINGLE_MARKER,
    'densenet161':           SINGLE_MARKER,
    'densenet169':           SINGLE_MARKER,
    'mobilenetv2_050':       SINGLE_MARKER,
    'mobilenetv2_100':       SINGLE_MARKER,
    'resnet18':              SINGLE_MARKER,
    'resnet34':              SINGLE_MARKER,
    'resnet50':              SINGLE_MARKER,
    'vit_small_patch16_224': SINGLE_MARKER,
})
...
TRANSF_METRICS_TO_MARKERS = rodict({
    # 'bh3_I+BEST':         's',
    # 'bh3a_I+BEST':        'D',
    'linear_pooling_per_score_dataset': 's',
    'imagenet':           '*',
    'ncti_score':         'd',
...
TRANSF_METRICS_TO_NAMES = rodict({
    ...
    'linear_pooling_per_score_dataset': 'Linear per Scorer/Dataset',
...
TRANSF_METRICS_TO_LEGEND = rodict({
    # 'bh3_I+BEST':         'BtB',
    'linear_pooling_per_score_dataset': 'Lin. per Scorer/Dataset',
    # 'bh3a_I+BEST':        'BtB/A',
    'imagenet':           'ImageNet',
    'ncti_score':         'NCTI',
    'etran_energy_score': 'ETran',
...
def create_subplot(ax, x, y, c, m, *, stat=None, title=None, first=False, ideal=False):
    x = np.asarray(x)
    y = np.asarray(y)
    c = np.asarray(c)
    m = np.asarray(m)

    # show ideal correlation
    if ideal:
        x_min, x_max = -5, 5
        y_min, y_max = -5, 5
        ax.plot([x_min, x_max], [y_min, y_max], linestyle='--', color='black', alpha=0.6)
    ...
    if stat:
        # old (0.05, 0.95)
        ax.text(0.05, 1.00, f'{stat["symbol"]} = {stat["mean"]*100:.0f} ',
    ...
    # Show x and y axes labels
    if first:
        y_label = 'rank|(|Test Metrics|)'.replace('|', '\u2009')
        x_label = 'rank|(|Transfer Scores|)\n(ranks computed per-dataset)'.replace('|', '\u2009')
...
def main():
    ...
    if DEBUG_ON_SUBSET:
        df = df[(df['transf_metric'].isin(['bh3_I+BEST', 'imagenet', 'ncti_score', 'tmi_score']))]
    ...
    SCALE = 4
    mm_to_in = (1./25.4)*SCALE
    fig = plt.figure(figsize=(82*mm_to_in, 15*mm_to_in))
    rows, cols = 1, 4  # Change based on desired layout
    ...
    for metric_id, transf_metric in enumerate(TRANSF_METRICS_TO_MARKERS):
        ...
        create_subplot(ax, all_x, all_y, all_c, all_m, stat=tau, title=title, first=(metric_id == 0), ideal=True)
    ...
    # render_legend(axes, ncol=1, axis_info=info, loc='upper center') # original 
    render_legend(axes, ncol=8, axis_info=info, loc='upper center') # original 
