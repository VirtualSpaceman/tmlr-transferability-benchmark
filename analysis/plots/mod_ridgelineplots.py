...
LAYOUT_FILE = os.path.join(SCRIPT_DIR, 'ridgelineplots.json')
...
if INPUT_DIRS == ['!']:
    print('WARNING: Simulating random data')

    # Generate random data
    # ... specifications
    n_prefixes = len(layout['rows'][0]['prefixes'])
    means = np.random.randn(n_rows, n_columns, n_prefixes)
    stddevs = np.random.exponential(scale=1.0, size=(n_rows, n_columns, n_prefixes))
    # ... computations
        ...
        for c, col in enumerate(layout['cols']):
            ...
            for p, prefix in enumerate(row['prefixes']):
                if prefix is None:
                    ...
                    mean, stddev = means[r, c, p], stddevs[r, c, p]
                    samples = np.random.normal(loc=mean, scale=stddev, size=30)
                    if PLOT_TYPE == 'hist':
                        kde = None
                    ...
                    y_max.append(ym)
                    maxll = samples.mean()
                    all_subsamples.setdefault((r, c), []).append(samples)
                    all_stats.setdefault((r, c), []).append({'mean': mean, 'maxll': maxll, 'mean-maxll': mean - maxll})
                    all_hdis.setdefault((r, c), []).append(az.hdi(samples, hdi_prob=0.95).tolist())
                    all_kdes.setdefault((r, c), []).append(kde)
                    all_histograms.setdefault((r, c), []).append((hist, bins))
    x_grids = [x_grid] * n_columns
...
fontsize = 12
fontsize_data = 12 if summarize_all else 13
row_factor = 1
col_factor = 1
...
def add_lines():
    ...
    for c in range(0, n_columns):
        this_group = layout['cols'][c]['group']
        if this_group != last_group and not changed:
            last_group = this_group
            x = col_lefts[c+1] - x_eps
            # Linha entre os grupos 
            fig.add_artist(matplotlib.lines.Line2D([x, x], [y_bottom, y_top], color='black', linewidth=mid_lines))
            # changed = True # desenha apenas a separacao entre o nome dos scorers e os datasets

        # # desenha uma linha para cada um dos datasets 
        # if c > 0:
        #     x_draw = col_lefts[c+1] - 0.5*(col_lefts[c+1]-col_lefts[c]) - x_eps
        #     fig.add_artist(matplotlib.lines.Line2D([x_draw, x_draw], [y_bottom, y_top - EPS_TOP_DELTA*y_eps], color='black', linewidth=mid_lines))
    
    # draw for combined column
    # x_draw = x_right - 0.5*col_widths[-1] - x_eps
    # fig.add_artist(matplotlib.lines.Line2D([x_draw, x_draw], [y_bottom, y_top - EPS_TOP_DELTA*y_eps], color='black', linewidth=mid_lines))

    last_group = layout['rows'][-1]['group']
    for r in range(1, n_rows):  # From bottom to top
        layout_r = n_rows - r - 1   # ...but the layout is from top to bottom
        this_group = layout['rows'][layout_r]['group'] # ...but the layout is from top to bottom
...
for r, row in enumerate(layout['rows']):  # Draw from top to bottom, so that the overlapping effect is correct
    ...
    for c, col in enumerate(layout['cols']):
        ...
        dists = [(p, prefix, get_stat(r, c, p)) for p, prefix in enumerate(row['prefixes']) if prefix is not None]
        
        # if len(dists) == 1:
        #     if row_group == 1: # ImageNet row , then the values comes first 
        #         dists.append((1, '', '-'))
            # elif row_group >= 3:
            #     dists.insert(0, (0, '', '-'))
            #     pos, pref, stat = dists[-1]
            #     dists[-1] = (pos+1, pref, stat)
            ...
            if d == 1 or summarize_all:
                ...
                if row_group == -1:
                    ...
                    if d == 1:
                        ...
                        # d = 2.3 -> Palavra BtB não passou da linha do meio 
                # if row_group == 1:
                    # d = 1
                # elif row_group >= 3:
                #     d = 2
                ...
                # Plot de todos os valores das celulas
                ...
                d += 1 # nao entendi 
                if row_group == -1:
                    fontsize_data = orig_font_size

# ... row headers
for r, row in enumerate(layout['rows'], start=1):
    if row["group"] == -1:
        continue
    ax = get_ax(r, 0)
...
# ... column headers
for c, col in enumerate(layout['cols'], start=1):
    ax = get_ax(0, c)
    # header_text = f"{col['header']}\n  . | BtB(.,I)"
    header_text = f"{col['header']}"
    ax.text(0.5, 0.5, header_text, horizontalalignment='center', verticalalignment='center', transform=ax.transAxes,
            fontsize=fontsize)
