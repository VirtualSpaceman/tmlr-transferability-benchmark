...
TRANSF_METRICS_TO_NAMES = rodict({
    ...
    'per_scorer_dataset_scorer': 'Linear per Scorer/Dataset',
...
TRANSF_METRICS_TO_LEGEND = rodict({
    # 'bh3_I+BEST':         'BtB',
    'per_scorer_dataset_scorer': 'Lin. per Scorer/Dataset',
    # 'bh3a_I+BEST':        'BtB/A',
    'imagenet':           'ImageNet',
    'ncti_score':         'NCTI',
    'etran_energy_score': 'ETran',
...
def create_subplot(ax, x, y, c, m, *, stat=None, title=None, simple=False):
    x = np.asarray(x)
    y = np.asarray(y)
    c = np.asarray(c)
    m = np.asarray(m)
    ...
    for marker in markers:
        ...
        c_subset = c[subset]
        if simple:
            diagonal = mlines.Line2D([0, 1], [0, 1], transform=ax.transAxes, color='#CCCCCC', linestyle=':',
                                     linewidth=0.5, zorder=0)
            ax.add_line(diagonal)
            ax.scatter(x_subset, y_subset, c='blue', marker='o', alpha=0.333, zorder=1)
        else:
...
def render_legend(axes, *, marker_items=MODEL_TO_MARKERS, marker_labels=MODEL_TO_LEGEND, marker_title=r'$\it{Models}$',
                  ...
                  ncol=2, axis_info=None, loc='center', simple=False):
...
def main():
    ...
    parser.add_argument('--simple', action='store_true', help='Simplify plot, removing colors and symbol differences')
    ...
    for transf_metric in TRANSF_METRICS_TO_MARKERS:
        ...
        create_subplot(ax, all_x, all_y, all_c, all_m, stat=tau, title=title, simple=SIMPLE)
    ...
    render_legend(axes, ncol=1, axis_info=info, loc='upper center', simple=SIMPLE)
    for ax in axes:
        ax.axis('off')
